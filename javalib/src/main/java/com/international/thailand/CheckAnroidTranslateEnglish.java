...
public class CheckAnroidTranslateEnglish {
     ...
     * 中英文按行一一对应
     */
    public static final String TRANS_CHINESE_PATH = "D:\\translateTemp\\replace2018.06.19\\translate_after\\0611zh";

    /**
     * 翻译回来的英文文件路径
     * 中英文按行一一对应
     */
    public static final String TRANS_ENGLISH_PATH = "D:\\translateTemp\\replace2018.06.19\\translate_after\\0611en";
        ...
        for (StringXmlBean bean : checkBeanList) {
            ...
            if (!file1.exists()) {
                ...
                if (files.length >= 1) {
                    ...
                    if (nodeKeyFile) {
                        //dom解析读取文件并执行添加
                        replaceStringByDom(files[0], key, value);
                    }else{
     ...
     * @param value
     */
    private void replaceStringByDom(File file, String key1, String value1) {
        if (!file.exists()) {
            return;
        }
